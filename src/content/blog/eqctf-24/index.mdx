...
Let's try to understand it bit by bit. By default, if we send it any values for example just an `empty string`. These response headers will be included in the output for sure.
...
However, only of them meet the conditions, the second response header that is included is not even inside the list of condition :T
